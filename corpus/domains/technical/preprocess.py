"""Preprocess technical text into individual sentences.

Usage:
    python -m corpus.domains.technical.preprocess
"""
...
ABBREVS = {
    ...
    "etc", "inc", "ltd", "vol", "no", "fig", "approx", "e.g", "i.e",
}
...
def process_source(source_dir: Path, source_name: str) -> list[dict]:
    sentences = []
            ...
            for line in f:
                ...
                for para in text.split("\n"):
                    ...
                    for sent in split_sentences(para):
                        sentences.append({"text": sent, "source": source_name, "domain": "technical"})
    return sentences
...
def main():
    ...
    for source_dir in sorted(OUTPUT_DIR.iterdir()):
        if source_dir.is_dir():
            raw = process_source(source_dir, source_dir.name)
            print(f"  {source_dir.name}: {len(raw)} raw sentences", flush=True)
