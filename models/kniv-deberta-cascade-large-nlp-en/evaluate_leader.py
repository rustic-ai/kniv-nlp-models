"""Evaluate leader model on all 5 heads: POS, NER, SRL, DEP (biaffine), CLS.

Usage:
    uv run python models/kniv-deberta-cascade-large-nlp-en/evaluate_leader.py \
        --model-dir models/kniv-v3.2
"""
...
# ── Viterbi decode ────────────────────────────────────────────
def viterbi_decode(logits: torch.Tensor, tag_map: dict[str, int]) -> list[int]:
    """Constrained Viterbi decoding for BIO sequences."""
    tags = list(tag_map.keys())
    T, C = logits.size()
    scores = logits.log_softmax(dim=-1)

    # Build transition mask: valid[i][j] = True if tag i -> tag j is valid
    valid = torch.ones(C, C, dtype=torch.bool)
    for i, ti in enumerate(tags):
        for j, tj in enumerate(tags):
            if tj.startswith("I-"):
                prefix = tj[2:]
                if not (ti == f"B-{prefix}" or ti == f"I-{prefix}"):
                    valid[i][j] = False

    # Viterbi forward
    viterbi = torch.full((T, C), -1e9)
    backptr = torch.zeros(T, C, dtype=torch.long)
    viterbi[0] = scores[0]
    # Can't start with I-
    for j, tj in enumerate(tags):
        if tj.startswith("I-"):
    ...
    for t in range(1, T):
        for j in range(C):
            candidates = viterbi[t - 1] + scores[t, j]
            candidates[~valid[:, j]] = -1e9
            best = candidates.argmax()
            viterbi[t, j] = candidates[best]
            backptr[t, j] = best
...
class TokenDataset(Dataset):
    """For POS and NER token-level tasks."""
    ...
    def __getitem__(self, idx):
        ...
        words = ex["words"]
        tags = ex.get(self.label_key, ex.get("ner_tags", ex.get("pos_tags", [])))
...
class SRLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # predicate_token_idx: FIRST subword of predicate word only (matches SRLEncoderModel)
        pred_token_idx = 0
        aligned, prev = [], None
    ...
    def __getitem__(self, idx):
        ...
        for k, wid in enumerate(word_ids):
            if wid is not None and wid != prev and wid < len(heads):
                ...
                if hw == -1:
                    head_labels[k] = k
                elif hw in word_to_token:
                    head_labels[k] = word_to_token[hw]
...
def main():
    ...
    parser.add_argument("--model-dir", default="models/kniv-v3.2")
    ...
    # NER (BiLSTM version — for NER eval)
    ...
    ner_head_lstm = nn.Sequential(nn.LayerNorm(H + len(pos_labels)), nn.Linear(H + len(pos_labels), H),
                                  nn.GELU(), nn.Dropout(0.1), nn.Linear(H, len(ner_labels)))
    ner_head_lstm.load_state_dict(state["ner_head_lstm"])

    # NER (original version — for DEP cascade, since DEP was trained before NER+BiLSTM)
    ner_head_orig = nn.Sequential(nn.LayerNorm(H + len(pos_labels)), nn.Linear(H + len(pos_labels), H),
                                  nn.GELU(), nn.Dropout(0.1), nn.Linear(H, len(ner_labels)))
    ner_head_orig.load_state_dict(state["ner_head"])

    # SRL (BiLSTM)
    srl_lstm = nn.LSTM(H, 256, num_layers=1, bidirectional=True, batch_first=True)
    srl_lstm.load_state_dict(state["srl_lstm"])
    srl_proj = nn.Linear(512, H)
    srl_proj.load_state_dict(state["srl_proj"])
    ...
    modules = [encoder, pred_embedding, pos_scalar_mix, pos_head,
               ner_scalar_mix, ner_lstm, ner_proj, ner_head_lstm, ner_head_orig,
               srl_lstm, srl_proj, srl_classifier,
    ...
    with open(DATA_DIR / "ud_dev.json") as f:
        ud_dev = json.load(f)
    ner_dev_path = DATA_DIR / "ner_spanmarker_dev.json"
    if not ner_dev_path.exists():
        ner_dev_path = DATA_DIR / "ner_dev.json"
    with open(ner_dev_path) as f:
        ner_dev = json.load(f)
    print(f"NER dev: {ner_dev_path.name}")
    ...
    print(f"\nEval sets: POS/DEP={len(ud_dev)}, NER={len(ner_dev)}, SRL={len(srl_dev)}, CLS={len(cls_dev)}")
    print("=" * 60)
    ...
    # ── POS ───────────────────────────────────────────────
    print("\n[POS] Evaluating...")
    pos_loader = DataLoader(TokenDataset(ud_dev, tokenizer, pos_map, "pos_tags"), batch_size=BS, collate_fn=collate)
    correct, total = 0, 0
    with torch.no_grad():
        for batch in tqdm(pos_loader, desc="POS"):
            ...
            h = pos_scalar_mix(out.hidden_states)
            preds = pos_head(h).argmax(dim=-1).cpu()
    ...
    ner_loader = DataLoader(TokenDataset(ner_dev[:2000], tokenizer, ner_map, "ner_tags"), batch_size=BS, collate_fn=collate)
    all_g, all_p = [], []
    with torch.no_grad():
        for batch in tqdm(ner_loader, desc="NER"):
            ...
            logits = ner_head_lstm(torch.cat([adapted, pos_p], dim=-1)).cpu()
            labs = batch["labels"]
    ...
    # ── SRL ───────────────────────────────────────────────
    print("\n[SRL] Evaluating...")
    srl_loader = DataLoader(SRLDataset(srl_dev, tokenizer, srl_map), batch_size=BS, collate_fn=collate)
    all_g, all_p = [], []
    with torch.no_grad():
        for batch in tqdm(srl_loader, desc="SRL"):
            B, S = batch["input_ids"].size()
            embed = encoder.embeddings(batch["input_ids"].to(device))
            # Single-token indicator (matches SRLEncoderModel.forward)
            ...
            # Call encoder.encoder directly (matches SRLEncoderModel training path)
            enc_out = encoder.encoder(
                hidden_states=embed.float(),
                attention_mask=batch["attention_mask"].to(device),
                output_hidden_states=False,
                return_dict=True,
            )
            h = enc_out.last_hidden_state
            lstm_out, _ = srl_lstm(h)
            adapted = srl_proj(lstm_out) + h
            logits = srl_classifier(adapted).cpu()
            labs = batch["labels"]
            for j in range(labs.size(0)):
                vi = (labs[j] != -100).nonzero(as_tuple=True)[0]
                if len(vi) == 0:
    ...
    # ── DEP ───────────────────────────────────────────────
    print("\n[DEP] Evaluating...")
    dep_loader = DataLoader(BiaffineDEPDataset(ud_dev, tokenizer, deprel_map), batch_size=BS, collate_fn=collate)
    ...
    with torch.no_grad():
        for batch in tqdm(dep_loader, desc="DEP"):
            ...
            pos_h = pos_scalar_mix(out.hidden_states)
            pos_p = torch.softmax(pos_head(pos_h), dim=-1)
            # DEP was trained with original NER head (no BiLSTM) for cascade
            ner_h = ner_scalar_mix(out.hidden_states)
            ner_p = torch.softmax(ner_head_orig(torch.cat([ner_h, pos_p], dim=-1)), dim=-1)
            proj_h = dep_proj(torch.cat([dep_h, pos_p, ner_p], dim=-1))
            arc_scores, label_scores = dep_biaffine(proj_h)
            head_labels, rel_labels_gt = batch["head_labels"], batch["rel_labels"]
            ...
            both = (pred_heads[vmask] == head_labels[vmask]) & (pred_rels[vmask] == rel_labels_gt[vmask])
    ...
    print(f"  CLS Macro F1: {cls_mf1:.4f}")
    print(f"\n{classification_report(all_g, all_p, target_names=CLS_LABELS, digits=3)}")

    # ── Summary ───────────────────────────────────────────
    print("\n" + "=" * 60)
    print("LEADER v3.2 EVALUATION SUMMARY")
    print("=" * 60)
    print(f"  POS:  {pos_acc:.4f}  (accuracy)")
    print(f"  NER:  {ner_f1:.4f}  (F1, seqeval)")
    print(f"  SRL:  {srl_f1:.4f}  (F1, seqeval, excl V)")
    print(f"  DEP:  {uas:.4f}  (UAS) / {las:.4f} (LAS)")
    print(f"  CLS:  {cls_mf1:.4f}  (macro F1)")
    print("=" * 60)

    # Compare with metadata
    with open(f"{args.model_dir}/metadata.json") as f:
        meta = json.load(f)
    print("\nDelta vs metadata:")
    reported = meta["heads"]
    for head, (score, metric) in [("pos", (pos_acc, "accuracy")), ("ner", (ner_f1, "F1")),
                                   ("srl", (srl_f1, "F1")), ("dep", (uas, "UAS")),
                                   ("cls", (cls_mf1, "macro_f1"))]:
        if head in reported:
            rep = reported[head]["score"]
            delta = score - rep
            status = "✓" if abs(delta) < 0.01 else ("↑" if delta > 0 else "↓")
            print(f"  {head.upper():>4}: {rep:.4f} → {score:.4f}  ({delta:+.4f}) {status}")
