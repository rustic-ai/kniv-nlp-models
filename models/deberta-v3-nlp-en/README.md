...
This is the **student** model, trained via knowledge distillation from [kniv-deberta-v3-large-nlp-en](https://huggingface.co/dragonscale-ai/kniv-deberta-v3-large-nlp-en) (304M parameter teacher). Shipped as an INT8-quantized ONNX model for efficient CPU inference.
...
| **Parameters** | 44M (6 layers, 768 hidden) |
| **Teacher** | kniv-deberta-v3-large-nlp-en (304M, DeBERTa-v3-large) |
...
- **Half the parameters** (44M vs 82M) at the same depth (6 layers, 768 hidden)
...
```
DeBERTa-v3-small encoder (shared, 44M params)
  +-- NER head: Linear(768, 37)   -- per-token BIO entity tags
  +-- POS head: Linear(768, 17)   -- per-token UPOS tags
  +-- Dep head: Linear(768, ~N)   -- per-token dep2label composite tags
  +-- CLS head: Linear(768, 7)    -- per-sequence sentence type (from [CLS] token)
```
...
| **Dep** | ~800--1200 (dep2label composite tags) | Composite labels | UD English EWT |
| **CLS** | 7 (sentence type) | Flat labels | Bootstrap labels from UD EWT + kniv corpus |
...
| `command` | Imperative or request |
| `greeting` | Social opener |
| `filler` | Reaction or backchannel |
| `acknowledgment` | Agreement or confirmation |
...
| Parameter | Value |
|---|---|
| Teacher | kniv-deberta-v3-large-nlp-en (DeBERTa-v3-large, 304M params) |
| Temperature (T) | 3.0 |
...
- **CLS labels are bootstrapped.** Sentence type labels are generated by rule-based heuristics on UD EWT text, not human-annotated. Expect lower accuracy on ambiguous or non-standard sentence types.
...
- **Distillation ceiling.** As a student model, performance is bounded by the teacher (kniv-deberta-v3-large-nlp-en). The student may underperform the teacher on tail entities and rare dependency constructions.
